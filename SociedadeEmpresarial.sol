...
contract SociedadeEmpresarial {
    ...
    //administrador [] listaDeAdministradores;
    //conselheiroFiscal [] listaDeConselheiros;
    alteracaoDoContratoSocial [] listeDeAlteracoesDoContratoSocial;
    //assembleiaGeral [] listaDeAssembleias;
    //resultadoFinanceiro [] listaDeResultados;
    ...
    //struct conselhoFiscal
    
    //struct assembleiaGeral
    
    struct alteracaoDoContratoSocial {
        bool votacaoEncerrada;
        bool resultadoDaVotacao;
        uint256 capitalSocialAnterior;
        uint256 novoCapitalSocial;
        uint256 numeroDeAcoesAnterior;
        uint256 novoNumeroDeAcoes;
        uint256 dataDaOperacao;
    }
    
    //struct assembleiaGeral
    
    //struct resultadoFinanceiro
    ...
    //function votarAlteracaoDoContratoSocial()
    
    function registrarAcionista(string memory _nome, uint256 _cpf, address payable _wallet, uint256 _numeroDeAcoes) public {
        require (msg.sender == owner);
        listaDeAcionistas.push(acionista(_nome, _cpf, _wallet, _numeroDeAcoes, true));
    }
    
    //function distribuirAcoes()
    
    //function assembleiaGeral ()
    
    //function votarAssembleiaGeral()
    
    //function registrarResultado()
    
    //function aprovarResultadoFinanceiro()
